"""
ORM 모델과 실제 DB 스키마 1:1 대조 및 입출력 검증 모듈.
...
class AppCoreModelsTest(unittest.TestCase):
    """
    ORM 모델(정답)과 실제 DB 구조(실제 출력) 1:1 대조 단위 테스트 클래스.
    """
    ...
    def assert_parameters_complete(self, func, input_dict: dict) -> None:
        ...
        for param in required_params:
            self.assertIn(
                param, input_dict,
                f"매개변수 누락 오류: '{param}' 항목이 input 데이터에명시되지 않았습니다."
            )
    ...
    def test_compare_orm_expected_vs_db_actual(self) -> None:
        """
        ORM 정답(Expected)과 실제 DB 출력(Actual)을 1:1 대조 비교하는 테스트.
        """
        ...
        # 1. 테이블 수 및 명칭 대조
        print("\n==========================================")
        print("[테스트] ORM 모델(정답) vs 실제 DB(출력) 1:1 스키마 대조")
        print("==========================================")
