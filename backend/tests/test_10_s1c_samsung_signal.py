"""
AlgoFinder backend/tests/test_08_s1c_samsung_signal.py
20260226부터 최신 거래일까지 삼성전자(005930) DB 데이터를 S1cMACrossAdaptiveStrategy 백엔드 전략 엔진에 대입하고,
매수(BUY) 및 매도(SELL) 시그널 출력 결과를 1:1로 정밀 대조하는 단위 테스트 모듈.
"""
...
class Test08S1cSamsungSignal:
    """삼성전자 S1c 20일선 적응형 전략 시그널 1:1 정밀 대조 테스트 클래스."""
    ...
    def test_08_s1c_samsung_signal_calculation(self):
        """
        20260226부터 최신 거래일까지 삼성전자(005930) 주가/수급 데이터를 불러와
        S1cMACrossAdaptiveStrategy 전략 엔진의 시그널 산출 결과를 1:1로 비교 검증한다.
        """
        ...
        print(f"\n[Inspect 매개변수 검증] S1cMACrossAdaptiveStrategy.calculate_indicators params: {params}")
        ...
        # 4. 백엔드 S1cMACrossAdaptiveStrategy 전략 엔진 계산 실행
        ...
        print("\n=========================================================================================")
        print(f"[삼성전자(005930) {start_date} ~ 최신 거래일 S1c 전략 시그널 1:1 정밀 검증 표]")
        print("=========================================================================================")
        print(f"{'날짜 (date)':<12} | {'종가 (close)':<10} | {'5일선(sma5)':<10} | {'20일선(sma20)':<10} | {'S1c 시그널'}")
        print("-----------------------------------------------------------------------------------------")

        for idx, row in df_result.iterrows():
            d = row['date']
            close_val = f"{int(row['close_price']):,}" if pd.notnull(row['close_price']) else "-"
            sma5_val = f"{row['sma5']:.1f}" if pd.notnull(row['sma5']) else "-"
            sma20_val = f"{row['sma20']:.1f}" if pd.notnull(row['sma20']) else "-"
            ...
            print(f"{d:<12} | {close_val:<10} | {sma5_val:<10} | {sma20_val:<10} | {sig_str}")

        print("=========================================================================================")
        print(f"[1:1 정답/실제 비교 Summary]")
        print(f"  - 조회 총 거래일수: {len(df_result)}일")
        print(f"  - [BUY] S1c 매수 시그널 발생 건수: {len(buy_dates)}건 (날짜 목록: {buy_dates if buy_dates else '없음'})")
        print(f"  - [SELL] S1c 매도 시그널 발생 건수: {len(sell_dates)}건 (날짜 목록: {sell_dates if sell_dates else '없음'})")
        print("=========================================================================================\n")
