...
코딩 룰은 Coding_Rule.md를 가장 우선 적용한다.
...
### [20260828]_docs 파일 변경 색상 표기

* 적용 대상: 소스 코드(`.py`/`.js`/`.html`/`.css` 등)와 `CLAUDE.md`만 제외하고, `.md` 문서를 수정할 때는 모두 적용한다.
...
* 표(table) 셀 안에서도 동일하게 셀 텍스트 단위로 `<span>`을 적용한다.
* 신규 문서 최초 생성 시에는 색상 표기를 하지 않는다(전체가 신규이므로).
* 계획 문서 파일명: `docs/`에 계획 문서를 `.md` 포맷으로 작성할 때는 파일 이름을 `세션이름.md`로 한다.
