...
## backend/app/api/routes_paper.py (457줄)
...
## backend/app/services/proposal_advisor.py (<span style="color:red">254줄</span>)
...
- <span style="color:red">(2026-08-29) 성능 A+C: 기준일마다 재로딩하던 것을 넓은 창(워밍업+FORWARD)
  1회 로드·1회 지표 계산 후 창 내 슬라이스로 처리하도록 `_load` 재구성 + `_covers`/`_resolve_dates`
  헬퍼 추가. 약 211 → 254줄. 캐시 키는 `(모드, 데이터버전)`으로 축소(`proposal_advisor_cache.py`).</span>
...
- <span style="color:red">(2026-08-29) `load_market_dataframe`에 `warmup_days` 선택 인자 추가(기본 200,
  기존 호출부 불변). ProposalAdvisor가 창 시작에 워밍업을 이미 반영하고 `warmup_days=0`으로 호출.</span>
